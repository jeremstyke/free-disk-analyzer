...
## VPN, option 2: reseller-backed integrated VPN (much further out, per Bob, logged only, nothing started)

Explicitly not part of the v2 vision above, per Bob on 2026-09-09: this is a "much later" item, well past v2, kept separate so it doesn't get pulled forward by accident when v2 work starts.
...
## Startup Manager added (per Bob, 2026-09-10), VPN tab deferred pending clarification
...
Bob's original request also included a VPN "teasing" tab: a page promising immediate, unlimited VPN access for a real 24.99€ Gumroad payment (or ad-view-earned "VPN hours"), while the actual VPN backend doesn't exist yet. Declined to build the payment/ad-reward mechanics for a non-functional product, that's charging real money or collecting ad views against a promise the app can't currently deliver. Offered instead: a "coming soon" informational page (no payment), the existing NordVPN affiliate CTA (a real, working product), and optionally a clearly-labeled "support future development" donation link to Gumroad (framed as support, not as purchasing working VPN access). Bob deferred the whole VPN tab for a later conversation, nothing built yet on that front. If revisited, keep the framing distinction above: informational/donation language is fine, promising a working feature that doesn't exist in exchange for money is not, regardless of how it's labeled ("teasing", "offre de lancement", etc.).
